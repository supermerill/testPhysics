...
public class JointPose extends Joint {
	...
	public void updateForce(long instant, long dt) {
		...
		for (idx = 1; idx < listVector.size() && !(findPosPos && findPosNeg && findNegPos && findNegNeg); idx++) {
			float dot = normalePlan.dot(listVector.get(idx));
			float dot2 = normalePlan2.dot(listVector.get(idx));
			if (dot < 0) {
				if (dot2 < 0) {
					findNegNeg = true;
				}else{
		...
		System.out.println(findNegNeg+" <0< "+findNegPos +" "+findPosNeg+" < "+findPosPos);
...
//			// TODOAFTER / normaleProjPlan.lengthsquared suffit je pense (penser
//			// à update la boucle aussi)
//			float maxAngleLow = normaleProjPlan.dot(projeteVect) / projeteVect.length();
...
//				float angleLow = normaleProjPlan.dot(projeteVect) / projeteVect.length();
...
//			}
			...
			int idxNear = 0;
			float distanceNear = Math.abs(normaleProjPlan.dot(listVector.get(0)));
			for (idx = 1; idx < listVector.size(); idx++) {
				float dot = Math.abs(normaleProjPlan.dot(listVector.get(idx)));
				if (dot < distanceNear) {
					distanceNear = dot;
					idxNear = idx;
				}
			...
			Vector3f normalePlanP1 = sumForces.cross(listVector.get(idxNear)).crossLocal(sumForces)
					.normalizeLocal();
			System.out.println("normalePlanP1 : "+normalePlanP1);

			// find the "farest" in "front" (normalized by distance) => ie big angle
			//where is front? wtf? neg? pos?
			int idx2 = 0;
			float maxRatio = 0;
			for (idx = 0; idx < listVector.size(); idx++) {
				if(idx == idxNear) continue;
				float val = listPoint.get(idx).dot(normalePlanP1);
				//FIXME: distance dans le plan de sumForces?
				System.out.println("Idx: "+idx+", val: "+val+", dist : "+listPoint.get(idx).distance(point1));
			...
			System.out.println("idxSecond : "+idx2+" : "+listPoint.get(idx2));

			// if found, it's our second point! now we our points of rotation,
			// our axe and we can set the direction & amplitude of the force

			//check if the origin is really in the front of this plane (if not, it's in equilibrium state!)
			Vector3f rotationXforce = point1.subtract(point2).crossLocal(sumForces);
			...
			System.out.println("point1.dot(point2) : "+point1.dot(point2));
//			if( signTest<0 &&  ){
//				//equilibrium!
//			}
		...
		//check si la nouvelle force (sur le nouvel axe de rotation) est bloqué par des points de contacts
		// comme une pierre coincé en deux autres pierre à son extrémité
		// si oui, bien transmettre la résultante à ces points-là
		
		//transmettre la force aux différents points de contacts (si pas déjà fait)
