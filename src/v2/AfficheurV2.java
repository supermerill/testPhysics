...
import v2.joint.JointPonctuel;
import v2.joint.JointPose;
import v2.joint.JointRotation;
import v2.collision.CollisionUpdater;
...
public class AfficheurV2 extends JComponent {
	...
	Color c = Color.BLACK;
	static long horloge = 0;
	
	Set<Integer> keyPress = new HashSet<>();

	public static void main(String[] args) {
		JFrame fenetre = new JFrame();
		final AfficheurV2 view = new AfficheurV2();
		
		Vector3d positionGravite = new Vector3d(0,-100,0);

		Forme f = new Forme("TRAP");
//		f.addPoint(new Vector3d(-15, -15, -6));
//		f.addPoint(new Vector3d(15, -15, -6));
//		f.addPoint(new Vector3d(-15, 15, -6));
//		f.addPoint(new Vector3d(15, 15, -6));
//		f.addPoint(new Vector3d(-3, 3, 6));
...
//		f.roundBBRayon = 30;
//		//f.pangulaire = new Quaternion().fromAngleAxis(FastMath.PI*32f/180, Vector3d.UNIT_Z);
...
//		f.doNotFaceCenter();
//		view.formes.add(f);
		f.addPoint(new Vector3d(-100, 20, -50)); //0
		f.addPoint(new Vector3d(-100, 20, 50)); //1
		f.addPoint(new Vector3d(100, 20, 50)); //2
		f.addPoint(new Vector3d(100, 20, -50)); //3
		f.addPoint(new Vector3d(-100, -20, -50)); //4
		f.addPoint(new Vector3d(-100, -20, 50)); //5
		f.addPoint(new Vector3d(100, -20, 50)); //6
		f.addPoint(new Vector3d(100, -20, -50)); //7
		...
		//f.pangulaire = new Quaternion().fromAngleAxis(FastMath.PI*32f/180, Vector3d.UNIT_Z);
		f.forces.add(new Vector3d(0.000000f,-9.81f,0).mult((float)f.mass));
//		f.acceleration.set(0.000000f,-0.00000981f,0);
		...
		for(Vector3d vect : f.points){
			vect.divideLocal(10);
		}
		for(Vector3d vect : f.normales){
			vect.divideLocal(10);
		}
		f.doNotFaceCenter();
		f.computeNormales();
		...
		f = new Forme("SOL");
		f.addPoint(new Vector3d(-200, 10, -100)); //0
		f.addPoint(new Vector3d(-200, 10, 100)); //1
		f.addPoint(new Vector3d(200, 10, 100)); //2
		f.addPoint(new Vector3d(200, 10, -100)); //3
		f.addPoint(new Vector3d(-200, -10, -100)); //4
		f.addPoint(new Vector3d(-200, -10, 100)); //5
		f.addPoint(new Vector3d(200, -10, 100)); //6
		f.addPoint(new Vector3d(200, -10, -100)); //7
		f.triangles.add(new Triangle(f,1,0,4));
		f.triangles.add(new Triangle(f,4,5,1));
		f.triangles.add(new Triangle(f,2,1,5));
		f.triangles.add(new Triangle(f,5,6,2));
		...
		f.addPoint(new Vector3d(0, 0, 0)); //8
		f.addPoint(new Vector3d(0, 0, 0)); //9
//		
		f.addPoint(new Vector3d(-33, 10, 0)); //10
		f.addPoint(new Vector3d(20, 10, 20)); //11
		f.addPoint(new Vector3d(20, 10, -20)); //12
		f.addPoint(new Vector3d(0, 55, 0)); //13
//		f.addPoint(new Vector3d(-133, 10, 0)); //10
//		f.addPoint(new Vector3d(-80, 10, 20)); //11
//		f.addPoint(new Vector3d(-80, 10, -20)); //12
//		f.addPoint(new Vector3d(-100, 100, 0)); //13
		...
		new Thread() {
			public void run() {
				...
				updater.movingObjects.addAll(view.formes);
				updater.allObjects.addAll(view.formes);
				for(Forme f : view.formes){
					f.time = horloge;
				}
				System.out.println("start horloge @ "+horloge);
				while (true) {
...
//					System.out.println("finish sleep: "+currentMs+" =?= "+nextMs);
					int diff = (int)((currentMs-previousMs) * slowdown);
//					System.out.println("diff: "+diff +" = "+currentMs+" - "+previousMs);
					...
					if(!view.keyPress.contains(KeyEvent.VK_SPACE)){
						...
						horloge +=diff;
						System.out.println("move horloge of "+diff+" => "+horloge);
					}
					
					System.out.println("NEW TURN ---------------------------------------------------------------------------------"+horloge);
					...
					//apply gravity
//					ArrayList<Vector3d> force = new ArrayList<>();
//					Vector3d posforce = new Vector3d(0,0,0);
					...
					if(view.keyPress.contains(KeyEvent.VK_Q)){
						maForme.forces.add(new Vector3d(20,20,0));
						maForme.pointApplicationForce.add(maForme.transfoMatrix.mult(new Vector3d(0, 0, 0)));
						maForme.physicUpdate = true;
						System.out.println("add force "+new Vector3d(20,20,0)+" @"+new Vector3d(0, 0, 0));
					}
					...
					if(view.keyPress.contains(KeyEvent.VK_Z)){
						maForme.forces.add(new Vector3d(0,20,0));
						maForme.pointApplicationForce.add(maForme.transfoMatrix.mult(new Vector3d(0, 0, 0)));
						maForme.physicUpdate = true;
						System.out.println("add force "+new Vector3d(0,20,0)+" @"+new Vector3d(0, 0, 0));
					}
					if(view.keyPress.contains(KeyEvent.VK_E)){
						maForme.forces.add(new Vector3d(0,20,0));
						maForme.pointApplicationForce.add(maForme.transfoMatrix.mult(new Vector3d(-100, -20, 0)));
						maForme.physicUpdate = true;
						System.out.println("add force "+new Vector3d(0,20,0)+" @"+new Vector3d(-100, -20, 0));
					}
...
//					}
					...
					for(Forme f : view.formes){
						System.out.println("aff: UF,UV forme "+f +"@"+f.position);
						if(f.joint instanceof JointPose){
							...
							while (numPointToCheckAeff < joint.points.size()) {
									Vector3d point = joint.points.get(numPointToCheckAeff);
						...
						f.joint.updateForce(horloge, diff);
						f.joint.updateVitesse(horloge, diff);
...
//					updater.updateCollision(currentMs, diff);
					System.out.println("redoCollisionPrediction");
					updater.redoCollisionPrediction(horloge, diff);
					// ....
					System.out.println("resolveAllCollisionPrediction");
					updater.resolveAllCollisionPrediction(horloge, diff);
					System.out.println("resolveAllCollisionPrediction ended");
					
					//TODO: faire avancer le temps.
					for(Forme f : view.formes){
						System.out.println("f.joint.updatePos "+f +"@"+f.position +" == "+f.joint.f+"@"
								+f.joint.f.position);
						f.joint.updatePosition(horloge, (long)((horloge-f.time) * slowdown));
						System.out.println("f.joint.updatePos edned "+f +"@"+f.position +" == "+f.joint.f+"@"
								+f.joint.f.position);
					}
...
//				System.out.println("m = "+forme.transfoMatrix.mult(forme.points.get(tri.a), null));
				...
				Vector3d m = forme.transfoMatrix.mult(forme.points.get(tri.a), null).multLocal(taille);
				Vector3d n = forme.transfoMatrix.mult(forme.points.get(tri.b), null).multLocal(taille);
				Vector3d e = forme.transfoMatrix.mult(forme.points.get(tri.c), null).multLocal(taille);
			...
			if(forme.vitesse.lengthSquared() != 0){
				Vector3d m = forme.vitesse.mult(1000).addLocal(forme.position.toVec3f().multLocal(taille));
				Vector3d n = forme.position.mult(taille);
				g.drawLine(maxX + (int) (m.x + m.z / 2), maxY
						- (int) (m.y + m.z / 2), maxX + (int) (n.x + n.z / 2),
						maxY - (int) (n.y + n.z / 2));
			}
...
//			}
			...
			if(forme.vangulaire.lengthSquared() != 0){
				Vector3d m = forme.vangulaire.mult(1000).addLocal(forme.position.toVec3f().multLocal(taille));
				Vector3d n = forme.position.mult(taille);
				//draw the rotaion vector?
//				g.drawLine(maxX + (int) (m.x + m.z / 2), maxY
//						- (int) (m.y + m.z / 2), maxX + (int) (n.x + n.z / 2),
//						maxY - (int) (n.y + n.z / 2));
				for (Vector3d point : forme.points) {
					m = forme.transfoMatrix.mult(point, null).multLocal(taille);
					Vector3d om = m.subtract(forme.position.mult(taille));
					Vector3d rotVect = forme.vangulaire.mult(100).crossLocal(om);
			...
			if(forme.joint instanceof JointPose){
				for(Vector3d v : ((JointPose)forme.joint).points){
